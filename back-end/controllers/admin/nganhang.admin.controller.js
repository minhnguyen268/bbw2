...
class NganHangAdminController {
  ...
  static editNganHangUser = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const { soTaiKhoan } = req.body;

    const result = await LienKetNganHang.findOneAndUpdate(
      { _id: id },
      {
        $set: {
          soTaiKhoan,
  ...
  static deleteNganHangUser = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const result = await LienKetNganHang.deleteOne({
      _id: id,
    });
